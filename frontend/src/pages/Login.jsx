...
export function Login(){
  ...
  async function submit(e){ e.preventDefault(); setError(''); setLoading(true);
    try {
      ...
      login(token, email); navigate('/ideas');
  ...
  return <div style={styles.outer}>
    <form onSubmit={submit} style={styles.form}>
      <h2 style={{margin:'0 0 1rem'}}>Login</h2>
      ...
      <button disabled={loading} style={styles.primary}>{loading? 'Signing in...':'Sign In'}</button>
      <div style={styles.alt}>No account? <a style={styles.link} onClick={()=>navigate('/register')}>Register</a></div>
    </form>
  </div>;
}
